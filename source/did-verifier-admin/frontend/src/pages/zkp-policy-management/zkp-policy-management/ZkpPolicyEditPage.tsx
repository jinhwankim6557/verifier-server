...
import { getPolicy, putPolicy } from '../../../apis/zkp-policy-api';
...
const ZkpPolicyEditPage = (props: Props) => {
  const { id } = useParams();
  ...
  const handleCancel = () => {    
    navigate(`/zkp-policy-management/zkp-policy-management/${policyId}`);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await dialogs.open(CustomDialog, { 
        title: 'Success', 
        message: 'ZKP Policy has been updated successfully.', 
        isModal: true 
      }, {
        onClose: async () => navigate(`/zkp-policy-management/zkp-policy-management/${policyId}`, { replace: true }),
      });
    } catch (err) {
  ...
  return (
    <>
      ...
      {policyData && (
        <StyledContainer>
          <StyledTitle>ZKP Policy Edit</StyledTitle>
          <StyledInputArea>
            <TextField
              fullWidth
              required
...
export default ZkpPolicyEditPage;